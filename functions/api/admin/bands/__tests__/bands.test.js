...
describe('Admin bands API - smoke', () => {
  ...
  it('duplicate band name returns 409', async () => {
    ...
    const body = { eventId: ev.id, venueId: venue.id, name: 'SameBand', startTime: '18:00', endTime: '19:00' }
  ...
  it('conflict detection finds overlapping times at the same venue', async () => {
    ...
    // Existing band
    const body1 = { eventId: ev.id, venueId: venue.id, name: 'Band One', startTime: '18:00', endTime: '19:00' }
    const req1 = new Request('https://example.test/api/admin/bands', {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify(body1)
    })
    const r1 = await bandsHandler.onRequestPost({ request: req1, env })
    expect(r1.status).toBe(201)
  ...
  it('GET /api/admin/bands?event_id returns bands with venue and event names', async () => {
    ...
    const body = { eventId: ev.id, venueId: venue.id, name: 'List Band', startTime: '18:00', endTime: '19:00' }
