...
export async function onRequestPatch(context) {
  ...
  try {
    ...
    // Audit log
    await auditLog(env, currentUser.userId, 'user.updated', 'user', userId, {
...
export async function onRequestDelete(context) {
  ...
  try {
    ...
    await DB.prepare(`
      ...
      WHERE id = ?
    `).bind(currentUser.userId, userId).run();
    ...
    // Audit log
    await auditLog(env, currentUser.userId, 'user.deleted', 'user', userId, {
      email: user.email,
      role: user.role
    }, ipAddress);
