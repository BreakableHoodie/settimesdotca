...
async function main() {
  try {
    // Read bands.json
    const bandsPath = path.join(__dirname, '..', 'frontend', 'public', 'bands.json')
    ...
    const eventName = await question('Event name (e.g., "Long Weekend Band Crawl - October 2025"): ')
    const eventDate = bandsData[0]?.date || await question('Event date (YYYY-MM-DD): ')
    ...
    console.error(`Event will be ${isPublished ? 'published' : 'unpublished'}\n`)
    ...
    sql.push(`INSERT INTO events (name, date, slug, is_published) VALUES (${escapeSQL(eventName)}, ${escapeSQL(eventDate)}, ${escapeSQL(eventSlug)}, ${isPublished});`)
    sql.push('')

    // Insert venues
    sql.push('-- Insert venues')
    venues.forEach(venue => {
      sql.push(`INSERT INTO venues (name, address) VALUES (${escapeSQL(venue.name)}, NULL);`)
    })
    sql.push('')

    // Insert bands
    sql.push('-- Insert bands')
    sql.push('-- Note: event_id = 1 assumes this is the first event in the database')
    ...
    bandsData.forEach(band => {
      ...
      sql.push(
        `INSERT INTO bands (event_id, venue_id, name, start_time, end_time, url) VALUES (${eventId}, ${venueId}, ${escapeSQL(band.name)}, ${escapeSQL(band.startTime)}, ${escapeSQL(band.endTime)}, ${escapeSQL(url)});`
      )
    })
    ...
    console.error('     node database/migrate-bands-json.js > database/migration.sql')
    console.error('')
    console.error('  2. Execute with wrangler:')
    console.error('     wrangler d1 execute bandcrawl-db --file=database/migration.sql')
    console.error('')
    console.error('  3. Or execute locally for testing:')
    console.error('     wrangler d1 execute bandcrawl-db --local --file=database/migration.sql')
    console.error('')
