# Documentation Index

**Last Updated**: January 29, 2026

---

## 📚 Technical Documentation

### Core System

| Document | Description |
| -------- | ----------- |
| [BACKEND_FRAMEWORK.md](BACKEND_FRAMEWORK.md) | Architecture patterns and conventions |
| [DATABASE.md](DATABASE.md) | ER diagrams, schema, relationships |
| [D1_SETUP.md](D1_SETUP.md) | Database setup and configuration |
| [DEPLOYMENT.md](DEPLOYMENT.md) | Cloudflare Pages deployment guide |

### Project Guide

| Document | Description |
| -------- | ----------- |
| [CLAUDE.md](CLAUDE.md) | Project overview and AI assistant instructions |
| [SQL_SAFETY.md](SQL_SAFETY.md) | SQL injection prevention guidelines |
| [SESSION_MANAGEMENT.md](SESSION_MANAGEMENT.md) | Session timeout and auth |

### User Documentation

| Document | Description |
| -------- | ----------- |
| [USER_GUIDE.md](USER_GUIDE.md) | Event organizer guide |
| [ADMIN_HANDBOOK.md](ADMIN_HANDBOOK.md) | System administrator guide |
| [QUICK_START.md](QUICK_START.md) | 10-minute getting started |
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | Common issues and solutions |

### API & Integration

| Document | Description |
| -------- | ----------- |
| [API_DOCUMENTATION.md](API_DOCUMENTATION.md) | REST API reference |
| [api-spec.yaml](api-spec.yaml) | OpenAPI specification |

### Feature Implementation

| Document | Description |
| -------- | ----------- |
| [BAND_PROFILES_IMPLEMENTATION.md](BAND_PROFILES_IMPLEMENTATION.md) | Band profile pages |
| [IMPLEMENTATION_SPEC_IMAGE_UPLOAD.md](IMPLEMENTATION_SPEC_IMAGE_UPLOAD.md) | R2 image upload (pending) |
| [PERFORMANCE_RECOMMENDATIONS.md](PERFORMANCE_RECOMMENDATIONS.md) | Performance optimization |

---

## 📁 Subdirectories

| Directory | Contents |
| --------- | -------- |
| [admin/](admin/) | Admin panel documentation |
| [frontend/](frontend/) | Frontend-specific docs |
| [design-system/](design-system/) | Component usage and design patterns |
| [security/](security/) | Security audit and SQL injection docs |

---

## 🔍 Quick Reference

### For Development

1. Project overview: [CLAUDE.md](CLAUDE.md)
2. Architecture: [BACKEND_FRAMEWORK.md](BACKEND_FRAMEWORK.md)
3. Database: [DATABASE.md](DATABASE.md)
4. Testing: [TESTING.md](TESTING.md)

### For Deployment

1. Database setup: [D1_SETUP.md](D1_SETUP.md)
2. Deployment: [DEPLOYMENT.md](DEPLOYMENT.md)
3. Security: [SQL_SAFETY.md](SQL_SAFETY.md)

### For Users

1. Quick start: [QUICK_START.md](QUICK_START.md)
2. Full guide: [USER_GUIDE.md](USER_GUIDE.md)
3. Problems: [TROUBLESHOOTING.md](TROUBLESHOOTING.md)

---

## 📝 Related Files

| Location | Purpose |
| -------- | ------- |
| `/ROADMAP_TO_DEMO.md` | Active project roadmap |
| `/SECURITY.md` | Security practices and headers |
| `/README.md` | Project overview and local dev setup |
