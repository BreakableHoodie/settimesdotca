...
import { createTestDB, mockUsers, insertEvent, createDBEnv, insertBand, createTestEnv } from '../../../test-utils'
...
vi.mock('../../_middleware.js', () => ({
  checkPermission: async (request, env, level) => {
    ...
    if (level === 'admin' && role !== 'admin') {
      return {
        error: true,
        response: new Response(JSON.stringify({ error: 'Forbidden', message: 'Admin required' }), { status: 403, headers: { 'Content-Type': 'application/json' } }),
      }
    }
...
describe('Event API - handler integration', () => {
  it('onRequestPost creates an event and returns 201', async () => {
    ...
    const body = { name: 'Integration Event', slug: 'integration-event', date: '2025-12-20' }
  ...
  it('create validation fails for past dates and bad slug', async () => {
    ...
    const pastBody = { name: 'Past Event', slug: 'past-event', date: '2000-01-01' }
    const reqPast = new Request('https://example.test/api/admin/events', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'x-test-role': 'editor' }, body: JSON.stringify(pastBody)
    })
    const rPast = await eventsHandler.onRequestPost({ request: reqPast, env })
    expect(rPast.status).toBe(400)

    const badSlugBody = { name: 'Bad Slug', slug: 'Invalid Slug!', date: '2026-01-01' }
    const reqSlug = new Request('https://example.test/api/admin/events', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'x-test-role': 'editor' }, body: JSON.stringify(badSlugBody)
    })
  ...
  it('PATCH cannot change slug', async () => {
    ...
    const ev = insertEvent(rawDb, { name: 'NoSlugChange', slug: 'no-slug-change' })
  ...
  it('delete requires admin role', async () => {
    const rawDb = createTestDB()
    const env = { DB: createDBEnv(rawDb) }
    const ev = insertEvent(rawDb, { name: 'ProtectedDelete', slug: 'protected-delete' })
  ...
  it('duplicate endpoint copies bands and creates new event', async () => {
    ...
    const body = { name: 'Copy of Original', date: '2026-01-01', slug: 'original-copy' }
