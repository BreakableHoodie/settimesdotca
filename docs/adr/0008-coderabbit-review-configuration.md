...
- **IMP-008** (resolved by #663): `sqlfluff` and `markdownlint` are now enabled in `reviews.tools`. `.sqlfluff` declares `dialect = sqlite` (fixing the ANSI parse errors on `PRAGMA`, `AUTOINCREMENT`, `INSERT OR IGNORE`) and restricts to the `core` rule group with nine further exclusions (`LT01,LT02,LT05,LT08,LT12,AL03,AL08,CP02,CP03`) — each a formatting/casing preference that fired across a large share of the 53 existing migrations, not a SQLite correctness issue. `.markdownlint.json` starts from `"default": true` and disables seventeen rules (`MD001,MD012,MD013,MD018,MD022,MD024,MD029,MD031,MD032,MD033,MD034,MD035,MD036,MD041,MD047,MD051,MD060`) that fired against this repo's existing docs corpus — long lines, inline HTML in ADRs, code fences packed tightly under list items, and the ADR template's deliberate jump from `##` to `#####` for "Alternatives Considered" entries. `MD040` (fenced code blocks must specify a language) stays enabled per `.github/instructions/markdown-gfm.instructions.md`'s documented convention; its six pre-existing bare fences in `CLAUDE.md` and the ADRs were fixed directly (tagged `bash` or `text`) rather than disabling the rule — a CodeRabbit review finding on the #663 PR. `CLAUDE.md` and every file under `docs/adr/` are verified clean (0 violations) against this config; other pre-existing docs (`docs/BACKEND_FRAMEWORK.md`, `docs/DEPLOYMENT.md`, etc.) still carry bare fences MD040 will flag if touched — left as pre-existing debt outside this PR's scope, not something it introduced.
