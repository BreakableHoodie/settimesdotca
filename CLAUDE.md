...
- `make hooks` installs `.githooks/pre-push`, which warns at the first review in the window and blocks at `LIMIT` (update it only against a current footer; erring low blocks pushes the budget would allow). Deliberately POSIX `sh`, no `gh`/`jq`/network. **Its counter is per developer, not per repo** (`~/.cache/coderabbit/push-log`, shared with `wrhn-mcp`'s identical hook), because the allowance is — a per-repo log let 3 + 3 pushes across the two repos reach 6 against a limit of 5 on 2026-09-24 with neither hook noticing.
