...
- **IMP-008** (resolved by #663): `sqlfluff` and `markdownlint` are now enabled in `reviews.tools`. `.sqlfluff` declares `dialect = sqlite` (fixing the ANSI parse errors on `PRAGMA`, `AUTOINCREMENT`, `INSERT OR IGNORE`) and restricts to the `core` rule group with nine further exclusions (`LT01,LT02,LT05,LT08,LT12,AL03,AL08,CP02,CP03`) — each a formatting/casing preference that fired across a large share of the 53 existing migrations, not a SQLite correctness issue. `.markdownlint.json` starts from `"default": true` and disables eighteen rules (`MD001,MD012,MD013,MD018,MD022,MD024,MD029,MD031,MD032,MD033,MD034,MD035,MD036,MD040,MD041,MD047,MD051,MD060`) that fired against this repo's existing docs corpus — long lines, inline HTML in ADRs, code fences packed tightly under list items, and the ADR template's deliberate jump from `##` to `#####` for "Alternatives Considered" entries. Both configs are verified clean (0 violations) across every file they cover today; a future violation is a genuine new issue, not repo-wide noise.
